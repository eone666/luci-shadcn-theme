...
The promise the README makes — that any official shadcn theme can be dropped into
`theme/globals.css` unchanged — rests on three things (verified by building
against the official neutral, zinc, slate, stone and gray):
...
name the files (that is how `docs/img/` is produced).

Screenshots are needed because LuCI views render on the client — `curl` only sees
the theme's shell. The browser comes from the Playwright cache and nothing is
downloaded; if there is no cache, `npx playwright install chromium` makes one, or
`CHROME_PATH` can point at any Chrome. `LUCI_URL`, `LUCI_USER` and `LUCI_PASS`
point the same scripts at a real router.
...
scripts/
  tokens.mjs               the palette -> src/generated/tokens.css
  postprocess.mjs          Tailwind output -> a plain LuCI stylesheet
